...
from verl.utils.reward_score import math_utils


def char_count_reward_function(data_source, solution_str, ground_truth, extra_info=None):
    try:
        last_boxed_string = math_utils.last_boxed_only_string(solution_str)
        if last_boxed_string is None:
            return 0
        solution = math_utils.remove_boxed(last_boxed_string)
