...
try:
    from verl.utils.reward_score.xveirfy.xverify_reward import xverify_evaluate_single, DEFAULT_XVERIFY_CONFIG
...
def hybrid_math_grading(answer, ground_truth):
    ...
    # Layer 3: Limited SymPy (only for simple expressions)
    if is_simple_expression(answer) and is_simple_expression(ground_truth):
        return limited_sympy_check(answer, ground_truth)
...
def compute_score(solution_str: str, 
    ...
    if do_print:
        print(f"======== ENHANCED NUMINA MATH REWARD DEBUG (10% sample) ========")
        ...
        print(f"Using xverify: {use_xverify}, Using OTC: {use_otc}")
        if use_otc:
            tool_count = count_tool_calls(solution_str) if OTC_AVAILABLE else 0
    ...
    if use_xverify and XVERIFY_AVAILABLE:
        ...
        if answer is None:
            ...
            try:
                is_correct = hybrid_math_grading(answer, str(ground_truth))
    ...
    if return_dict:
        result = {
            ...
            "has_search_pattern": False,  # Math problems don't use search
